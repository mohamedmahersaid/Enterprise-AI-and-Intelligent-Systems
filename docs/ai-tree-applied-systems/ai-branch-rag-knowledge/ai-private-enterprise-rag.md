...
**Authorized is not trusted.** Security trimming decides which chunks a user may read; it says nothing about who wrote them. A chunk the user is entitled to see can still carry instructions from anyone who can write to the source - a shared wiki page, a supplier contract, an email archived into the library - and the model reads those instructions in the middle of the user's own request. Because this stack is air-gapped, the defence has to run locally, as two Spotlighting layers. Delimiting wraps each chunk in explicit start and end markers - carrying a random per-request tag, so a document cannot close a boundary it cannot guess - but it only fences the chunk; inside the fence the model still reads the injected text as ordinary prose, so treat it as the weaker layer. Datamarking goes further by interleaving a marker through the whole chunk text: replace every run of whitespace with a character that does not otherwise occur in the corpus, such as `^`, so `ignore all previous instructions` reaches the model as `ignore^all^previous^instructions`. Then state in the system prompt that delimited, marked text is reference data to quote or summarise, never instructions to follow. Where the same design runs on the managed Azure variant, add Prompt Shields document scanning in front of the prompt as well; the air-gapped build has no route to it. Log the id of every chunk placed in context, so an answer that followed an injected instruction can be traced back to the document that carried it.
...
flowchart TD
    ...
    I --> S[Delimit chunks + datamark text\nlog chunk ids in context]
...
- [Microsoft Learn: Defend against indirect prompt injection attacks](https://learn.microsoft.com/security/zero-trust/sfi/defend-indirect-prompt-injection) - Spotlighting (data marking plus a system-prompt instruction) as a layered, local defence for chunks a user is authorized to read but that carry third-party instructions.
- [arXiv (Hines et al., Microsoft): Defending Against Indirect Prompt Injection Attacks With Spotlighting](https://arxiv.org/abs/2403.14720) - Defines delimiting and datamarking separately: datamarking interleaves a marker through the input (for example in place of whitespace) and is the stronger of the two.
